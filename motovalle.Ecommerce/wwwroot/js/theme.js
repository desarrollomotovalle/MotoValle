	...
	var Core = {
		...
		initSlider: function(options){
			...
			if (slider) {
		        ...
		        $(".ui-slider-handle:last-of-type").append("<span class='max'>400</span>");
			}
		},

		initSliderMassey: function (options) {
			var slider = $('.slider.massey').length;
			if (slider) {
				jQuery(".slider.massey").slider({
					min: 0,
					max: 1000,
					values: [0, 1000],
					range: true,
					...
					stop: function (event, ui) {
						$("input.j-min.massey").val(ui.values[0]);
						$("input.j-max.massey").val(ui.values[1]);
					}
				});
				$(".ui-slider-handle:first-of-type").append("<span class='min'>0</span>");
				$(".ui-slider-handle:last-of-type").empty();
